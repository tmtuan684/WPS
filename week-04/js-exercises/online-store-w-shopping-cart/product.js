      ...
      <div class="card h-100 shadow-sm product-card">
        ...
        <div class="card-body d-flex flex-column">
          ...
          <div class="mt-auto">
            ...
            <button class="btn btn-sm btn-outline-primary w-100 mt-2" data-product-name="${p.name}" data-product-price="${p.price}" data-product-photo="${p.photo}">Add to Cart</button>
          </div>
        </div>
      </div>
    </div>`;
...
function renderProductsGrid(products, container) {
    ...
    products.forEach(p => { // For each product, make a product card

        const productDiv = document.createElement('div');
        productDiv.innerHTML = productCard(p);
        productDiv.getElementsByTagName('button')[0].addEventListener('click', addToCart); // Add product to card when clicked
        container.appendChild(productDiv); // Add product card to product list section on HTML page 
    });
}
